...
void main() {
  group('AppLockController', () {
    ...
    test('load retrieves lock status from repository without re-locking unlocked session', () async {
    ...
    test('setLockEnabled(true) authenticates before enabling', () async {
      ...
      final controller = AppLockController(AlwaysAuthenticates(), repository: repo);
    ...
    test('setLockEnabled(true) fails when device cannot authenticate', () async {
      final repo = InMemoryAppLockRepository();
      final controller = AppLockController(UnavailableAuthenticator(), repository: repo);

      final enabled = await controller.setLockEnabled(true);
      expect(enabled, isFalse);
      expect(controller.isLockEnabled, isFalse);
      expect(await repo.isLockEnabled(), isFalse);
      expect(controller.message, contains('Device authentication is not configured'));
    });

    test('setLockEnabled(true) fails when authentication is cancelled', () async {
      final repo = InMemoryAppLockRepository();
      final controller = AppLockController(
        ScriptedAuthenticator(AppAuthenticationResult.cancelled),
        repository: repo,
